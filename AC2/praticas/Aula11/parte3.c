  ...
  while(1)
  {
    if (store_temp == 30)
    {
      ...
      if(can_store)
      {
        ...
        eeprom_writeData(eeprom_readData(0x01), to_store);
        if (eeprom_readData(0x00) == 64 && eeprom_readData(0x01) == 66)
          eeprom_writeData(0x01, 0x02);
        else if (eeprom_readData(0x00) == 64 && eeprom_readData(0x01) != 66)
          eeprom_writeData(0x01, eeprom_readData(0x01)+1);
        else
        {
          eeprom_writeData(0x00, eeprom_readData(0x00)+1);
          eeprom_writeData(0x01, eeprom_readData(0x01)+1);
        }
      }
      store_temp = 0;
